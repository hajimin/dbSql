...
dept_test 테이블의 dname컬럼은 UNIQUE 제약이 있기 때문에 동일한 값이 들어갈 수 없다.
...
dname 컬럼과 loc컬럼이 동시에 동일한 값이어야만 중복으로 인식
밑에 두개의 쿼리는 데이터 중복이 아니므로 정상실행
INSERT INTO dept_test VALUES (99, 'ddit','daejeon');

INSERT INTO dept_test VALUES (98, 'ddit','대전');

아래쿼리는 UNIQUE 제약 조건에 의해 오류
INSERT INTO dept_test VALUES (98, 'ddit','대전');
...
CREATE TABLE emp_test(
        ...
        --CONSTRAINT 제약조건 명 제약조건 type 컬럼 REFERENCES 참조하는 테이블(컬럼)
...
부모 레코드를 (dept_test.deptno)를 참조라고 있는 자식 레코드 (emp_test.deptno)가
...
주관적인 의견 : default
1. 개발자가 테이블의 순서를 명확하게 알고 있어야지만 로직제어할 수 있음
    ==> 지우거나, 입력할 데이터의 순서를 알고 있어야함
2. 테이블 명세서가 정확하지 않으면 신규 개발자는 해당 내용을 모를 수가 있음
    (java 코드 + 테이블 내역 확인 필요)
    java 코드에는 dept테이블만 삭제하는 코드가 있는데 
    신기하게도 emp테이블의 데이터가 삭제되거나 null로 설정되는 경우를 볼 수 있음

CHECK 제약조건
컬럼의 값을 제한하는 제약 조건

emp 테이블에서 급여 정보(sal)를 관리할 때 sal컬럼의 값은 0보다 작은 값이 들어가는 것은 로직적으로 이상함.
...
sal 컬럼에 설정된 check 조건(sal > 0)에 의해 정상적으로 실행되지 않는다
...
NOT NULL 제약조건을 제외한 다른 제약 조건은 복사되지 않는다
용도
1.개발자 레벨의 백업
2.개발자 레벨의 테스트
...
이미 생성된 테이블을 수정 할 수도 있음
    1. 새로운 컬럼을 추가
        ...
        ****** 기존 생성되어 있는 컬럼 중간에는 새로운 컬럼을 추가할 수가 없다
        ==> 기존 테이블을 삭제하고 컬럼순서를 조정한 새로운 테이블 생성 DDL로 새롭게 테이블을 만들어야함.
            운영중이라면 이미 데이터가 쌓여있을 가능성이 굉장히 높음
            ==> 테이블을 생성할 때 신중하게 컬럼 순서를 고려

    2. 기존 컬럼 삭제, 컬럼 이름 변경, 컬럼 사이즈 변경,(타입도 제한적으로 변경가능)
        참조키가 걸려있는 테이블의 컬럼이름을 변경하더라도 참조하는 테이블에는 영향이 가지 않음
        (알아서 이름을 변경해준다)
        emp_test.deptno = dept_test.deptno참조
        dept_test,deptno 이름 변경하여 dept_test.dno로 수정하더라도 fk는 변경된 이름으로 잘 유지가 된다
...
컬럼 삭제
1. ALTER TABLE 테이블명 DROP (삭제할 컬럼명);
2. ALTER TABLE 테이블명 DROP COLUMN (삭제할 컬럼명);
...
DDL : CREATE...., ALTER.... ==> 트랜잭션 제어가 불가능 (취소가 안된다)/자동 커밋
