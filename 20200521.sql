...
호불호
10개, 1~2개 회사가 PL/SQL 를 선호
PL/SQL==> PL/SQL로 생성하는 것은 오라클 객체
          코드 자체가 오라클에 존재(오라클 객체니까)
          로직이 바뀌어도 일반 프로그래밍(java) 언어는 수정할 필요가 없음
          
SQL ==> SQL 실행은 일반언어에서 실행 (java)
...
연말정산 : 소득의 25% 를 신용카드 + 현금영수증 + 체크카드로 소비를 
          소비금액이 소비의 25%를 초과하는 금액에 대해서 신용카드는 공제 : 20%, 현금영수증은 30%, 체크카드 25%를 공제하는
          단, 공제금액이 300만원을 넘을수 없고
          단, 대중교통에 대해 추가적으로 100만원을 공제받을 수 있고
          단, 전통시장에 사용금에 대해서는 추가적으로 100만원을 공제받을 수 있다.
...
DECLARE
    ...
    v_dname VARCHAR2(14);
BEGIN    
    /*dept테이블의 10번 부서에 해당하는 부서번호, 부서명을 DECLARE 절에 선언한 두개의 변수에 담기*/
...
데이터 타입 참조
v_deptno, v_dname 두개의 변수 선언 ==> dept테이블의 컬럼 값을 담으려고 선언
...
데이터 타입을 직접 선언하지 않고 테이블의 컬럼타입을 참조하도록 선언할 수 있다
...
복합 변수
조회결과를 컬럼당 하나의 변수에 담는 작업 번거롭다 ==> 복합 변수 사용하여 불편함을 해소
...
%ROWTYPE
익명블럭을 dept 테이블의 10번 부서정보를 조회하여 %ROWTYPE으로 선언한 변수에
결과값을 저지하고 DBMS_OUTPUT.PUT_LINE을 이용하여 출력
...
dept테이블에서 dpetno, dname 두개 컬럼만 행단위로 저장하고 싶을때
...
복수행을 리턴할 때 비교
SELECT 결과가 복수행이기 떄문에 하나의 행 정보를 담을 수 있는 ROWTYPE 

TABLE TYPE : 여러 행을 저장 할 수 있는 타입
